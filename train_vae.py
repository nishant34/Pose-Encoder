...
def main():
    ...
    opt_vae = train_helper.get_optimizer_vae(model)
    ...
    train_summary_op = train_helper.init_vae_summary(model)
    ...
    for epoch in range(cfg.INIT_EPOCH+1, cfg.INIT_EPOCH+cfg.NUM_EPOCH+1):
        ...
        for step in tqdm(range(train_theta.shape[0] // cfg.TRAIN_BATCH_SIZE)):
            ...
            session.run([opt_vae], feed_dict=feed_dict)
            #[train_summary] = session.run([train_summary_op], feed_dict=feed_dict)
        ...
        if epoch % cfg.LOG_EPOCH == 0:
            ...
            print('{0:>4} '.format(epoch), '{0:>22} '.format(lr_), cfg.TRAIN_BATCH_SIZE, 'loss_autoencoder:{:.6f}'.format(loss_values_[0]), 'loss_vae:{:.6f}'.format(loss_values_[1]))#, 'loss_kl_div:{:.6f}'.format(loss_values_[1])
        ...
        if epoch % cfg.SAVE_MODEL_EPOCH == 0:
            encoder_saver.save(session, os.path.join(cfg.model_save_path_15, 'encoder'), global_step=epoch) 
            decoder_saver.save(session, os.path.join(cfg.model_save_path_15, 'decoder'), global_step=epoch) 
